...
df_train = pd.read_csv('data/glove_train_features.csv', encoding="ISO-8859-1")
X_train_ab = df_train.iloc[:, 3:-1]
...
for tmp in y_train:
    if tmp == 0:
        y_train_weight.append(1.309028344)
    else:
        y_train_weight.append(0.472001959)
y_valid_weight = []
for tmp in y_valid:
    if tmp == 0:
        y_valid_weight.append(1.309028344)
    else:
        y_valid_weight.append(0.472001959)

d_train = xgb.DMatrix(X_train, label=y_train, weight=y_train_weight)
d_valid = xgb.DMatrix(X_valid, label=y_valid, weight=y_valid_weight)

watchlist = [(d_train, 'train'), (d_valid, 'valid')]

bst = xgb.train(params, d_train, 5000, watchlist, early_stopping_rounds=50, verbose_eval=50)
print(log_loss(y_valid, bst.predict(d_valid)))
# bst.save_model(args.save + '.mdl')

print('Building Test Features')
df_test = pd.read_csv('data/glove_test_features.csv', encoding="ISO-8859-1")
...
sub.to_csv('predictions/baseline_glove_7d.csv', index=False)
